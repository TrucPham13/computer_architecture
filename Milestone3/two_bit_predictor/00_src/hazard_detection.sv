module hazard_detection (
  ...
  assign alu_hazard  = (i_wb_rd_wren && (i_wb_rd_addr != 5'd0) && 
                       ((i_wb_rd_addr == i_id_rs1_addr) || (i_wb_rd_addr == i_id_rs2_addr))) ? 1'b1 : 1'b0;
  assign load_hazard = (i_ex_is_load && (i_ex_rd_addr != 5'd0) && 
                       ((i_ex_rd_addr == i_id_rs1_addr) || (i_ex_rd_addr == i_id_rs2_addr))) ? 1'b1 : 1'b0;
  assign ctrl_hazard = i_ex_mispred;
  ...
  always_comb begin
    ...
    if (ctrl_hazard) begin
      ...
      o_if_flush = 1'b1;
      o_id_flush = 1'b1;
    end else if (alu_hazard || load_hazard) begin
      o_if_stall = 1'b1;
      o_if_flush = 1'b0;
      o_id_flush = 1'b1;
    end else begin
